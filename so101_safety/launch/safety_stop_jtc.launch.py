"""Depth-safety monitor + trajectory_safety_gate (JointTrajectory/JTC path).
...
used on the parent demo launch) and freezes the follower's JointTrajectory
stream via ``trajectory_safety_gate`` whenever a near object is detected.

Run:
  ros2 launch so101_safety safety_stop_jtc.launch.py
  ros2 launch so101_safety safety_stop_jtc.launch.py depth_image_topic:=/perception/depth
"""
